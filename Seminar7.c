...
HashTable initializareHashTable(int dimenisune){
    HashTable hash;
    hash.dim=dimenisune;
    hash.vector=(nod**)malloc(sizeof(nod*)*dimenisune);
    for(int i=0;i<dimenisune;i++){
        hash.vector[i]=NULL;
    }
...
int calculHash(int cod, const char* nume, int dimensiune){
    //hash codul este calculat in fct de un algoritm aleatoriu
    if(dimensiune>0){
...
void afisareGalerie(Galerie g){
    printf("%d. Intrare la galeria %s costa %.2f\n ", g.cod, g.nume, g.pretIntrare);
}
...
void main(){
    ...
    printf("Cautare galerie dupa cheie: \n");
    afisareGalerie(g);
}
